// WebAudio-синтезатор (без ассетов)
class AudioSys {
  ...
  toggleMute() { this.muted = !this.muted; if (this.master) this.master.gain.value = this.muted ? 0 : 0.42 }
  ...
  private noise(dur: number, vol: number, freq = 1000, q = 0.6, delay = 0, slideTo = 0) {
    ...
    f.type = 'bandpass'; f.frequency.setValueAtTime(freq, t0)
  ...
  teleport() { this.tone(180, 0.7, 'sawtooth', 0.14, 1400); this.tone(90, 0.8, 'square', 0.1, 700); this.noise(0.7, 0.1, 600, 0.5, 0.05, 4000); this.tone(1600, 0.3, 'sine', 0.12, -1200, 0.55) }
  ...
  switchOn() { this.tone(300, 0.08, 'square', 0.1, 200); this.tone(600, 0.1, 'square', 0.08, 300, 0.07) }
  doorOpen() { this.noise(0.5, 0.2, 300, 0.5, 0, 90); this.tone(90, 0.5, 'square', 0.1, 30) }
  bossRoar() { this.tone(70, 0.9, 'sawtooth', 0.22, 40); this.tone(55, 1.1, 'square', 0.14, -15, 0.1); this.noise(0.9, 0.14, 250, 0.4, 0.05, 90) }
  zoneBlip() { this.tone(1200, 0.05, 'sine', 0.06, 200) }
  alarm() { this.tone(700, 0.16, 'square', 0.09, -300); this.tone(700, 0.16, 'square', 0.09, -300, 0.22) }
}
