...
// 0 пол, 1 стена, 2 дверь (заперта), 3 рубильник, 4 хрупкая стена, 5 ловушка-излучатель
export type DungeonTile = 0 | 1 | 2 | 3 | 4 | 5
export interface Room { x: number; y: number; w: number; h: number; kind: 'entry' | 'hall' | 'side' | 'secret' | 'boss' }
...
export function dungeonTileAt(d: DungeonLayout, wx: number, wy: number): DungeonTile {
  ...
  if (tx < 0 || ty < 0 || tx >= d.cols || ty >= d.rows) return 1
...
/** Данж «заброшенный военный комплекс»: вход -> залы -> рубильники -> дверь босса -> арена + тайник */
export function makeDungeon(seed: number, ox: number, oy: number, tier: number): DungeonLayout {
  ...
  const d: DungeonLayout = {
    ...
    secretCache: { x: 0, y: 0, tier: tier + 1 }, hpx: 0, hpy: 0,
  }
  const room = (x: number, y: number, w: number, h: number, kind: Room['kind']): Room => {
  ...
  const sideL = room(8, 3, 6, 5, 'side')
  const sideR = room(8, 18, 6, 5, 'side')
  const corridor = room(15, 11, 5, 4, 'hall')
  const hallB = room(20, 8, 7, 10, 'hall')
